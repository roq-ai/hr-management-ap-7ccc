...
import { useRoqClient, useAdministratorFindFirst } from 'lib/roq';
...
function AdministratorEditPage() {
  ...
  const { data, error, isLoading, mutate } = useAdministratorFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: AdministratorInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.administrator.update({
        data: values as RoqTypes.administrator,
        where: {
          id,
        },
      });
