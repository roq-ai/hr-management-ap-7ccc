...
import { useRoqClient, useAdministratorFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<AdministratorInterface, unknown>;

interface AdministratorListPageProps {
...
export function AdministratorListPage(props: AdministratorListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['administrator_department.contains', 'job_title.contains'],
          ...params.filters,
        },
        'administrator',
      ),
  ...
  const { data, error, isLoading, mutate } = useAdministratorFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.administrator.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: AdministratorInterface) => {
    if (hasAccess('administrator', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/administrators/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'administrator_department', header: 'Administrator Department', accessorKey: 'administrator_department' },
    { id: 'job_title', header: 'Job Title', accessorKey: 'job_title' },
    { id: 'administrator_salary', header: 'administrator_salary', accessorKey: 'administrator_salary' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/administrators/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('administrator', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/administrators/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="administrator" />
        </Flex>

        {hasAccess('administrator', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/administrators/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
