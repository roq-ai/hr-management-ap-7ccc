...
import { useRoqPlatformUserProfiles, useEmployeeCount, useOwnerCount, useHrManagerCount } from 'lib/roq';

import { AdministratorListPage as ShowCaseList } from 'pages/administrators';
...
function DashboardPage() {
  ...
  const ownerStat = useOwnerCount();

  const hrManagerStat = useHrManagerCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Owners',
      result: ownerStat?.data,
      isLoading: ownerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Hr Managers',
      result: hrManagerStat?.data,
      isLoading: hrManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`administrators`} passHref legacyBehavior>
                  <Button
